...
class VintageRadioFirmware:
    ...
    def handle_track_finished(self):
        ...
        if getattr(self.hw, "check_track_finished_uart", None) and self.hw.check_track_finished_uart():
            ...
            # LOW the whole time; the module often sends 0x3D before BUSY rises. Ignoring UART
            # whenever BUSY was LOW made every real track end a no-op on PWM overlay / many boards.
